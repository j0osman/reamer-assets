...
Historical OHLCV data itself represents one of three things: bid prices, ask prices, or a midpoint between them. Which one it is changes which direction the spread needs to be applied — treating ask-quoted data as if it were midpoint data, for instance, doesn't just introduce a small error, it applies the spread adjustment in a direction that's already partially baked into the raw numbers, compounding rather than correcting the bias. Knowing what the underlying data actually represents, and configuring `ohlcv_type` to match it, is what keeps the spread adjustment pointed the right way instead of stacking on top of a data convention that already accounts for part of it.
