...
**A side effect of testing something else.** Sometimes the most interesting idea shows up while implementing a completely different one — a filter that seems to matter more than expected, a subset of the data where everything behaves differently, a pattern in the trades that got rejected rather than filled.
...
None of them are validated facts yet, regardless of how they arrived. A pattern from a desk's own chart-watching and a factor from a peer-reviewed paper are in exactly the same epistemic position the moment they're being considered for testing: both are claims that haven't been checked against realistic costs, the desk's own data, or the possibility that they're noise. A published result proves the idea worked somewhere, under some set of assumptions — not that it will work under this one.
